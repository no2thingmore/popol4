  ...
  return (
    <>
      <div className="KJH_inq_section">
        <div className="KJH_inq_route">Admin &gt; 고객 지원 &gt; 문의 내역</div>
          ...
          <div className="KJH_inq_top_section">
            ...
            <span className="KJH_inq_top_toggle_section">
              ...
              <button
                ...
                className="KJH_inq_top_close_toggle_section"
              >
                <img src={ArrowUp} alt="위쪽 화살표" />
              </button>
              {/* 내용 모두 열기 */}
              <button
                onClick={openAllToggles}
                className="KJH_inq_open_all_toggle"
              >
            ...
            <span className="KJH_inq_top_page_button_section">
              {[...Array(Math.ceil(inquiries.length / pageLimit)).keys()].map(
                (number) => (
                  <button
                    key={number}
            ...
            <tbody>
              {displayedPosts.map((item) => (
                <React.Fragment key={item.id}>
                  <tr
                    className="KJH_inq_contents_section"
                    onClick={() => toggleContent(item.id)}
                  >
                    <td className="KJH_inq_contents_id">{item.id}</td>
                    <td className="KJH_inq_contents_kind">
                      {tagsMapping[item.tags.toString()] || "알 수 없음"}
                    </td>
                    <td className="KJH_inq_contents_title">{item.title}</td>
                    <td className="KJH_inq_contents_created">
                    ...
                    <td
                      className={`KJH_inq_contents_status ${
                  ...
                  {showContents[item.id] && (
                    <>
                      <tr className="KJH_inq_coment_tr_1st">
                        <td />
                        <td colSpan="2" className="KJH_inq_contents_content">
                          {item.content}
                        </td>
                        <td />
                        <td />
                        ...
                        <tr className="KJH_inq_coment_tr_2nd">
                          ...
                          <td colSpan="2">
                            <textarea
                              ...
                              value={answers}
                            ></textarea>
                            <button onClick={() => submitAnswer(item.id)}>
                              작성
                            </button>
                          </td>
                          <td />
